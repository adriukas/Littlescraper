    ...
    <section class="card shadow-sm border-0 mb-4 text-center p-4">
        <form action="{{ route('run.scrape') }}" method="POST">
            ...
            <button type="submit" class="btn btn-success btn-lg px-5 shadow-sm">
                Scrape latest purchases in: {{ $botName }}
            </button>
        </form>
    </section>
    ...
    @if(is_array($purchases))
                    ...
                    <tbody>
                        @foreach($purchases as $item)
                            ...
                            <tr>
                                ...
                                <td><span class="badge bg-light text-dark border">{{ $item['item'] }}</span></td>
        ...
        @else
            <div class="alert alert-light text-center shadow-sm py-5 border">
                <i class="bi bi-bag-x text-success d-block mb-3" style="font-size: 2.5rem;"></i>
                <h5 class="fw-bold">No new purchases found</h5>
                <p class="text-muted">There were no new purchases from <strong>{{ $botName }}</strong> in the last 24 hours.</p>
            </div>
        @endif
    @endif
@endsection
